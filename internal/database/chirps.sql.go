...
const allChirpsByAuthor = `-- name: AllChirpsByAuthor :many
SELECT id, created_at, updated_at, body, user_id FROM chirps
WHERE user_id = $1
ORDER BY created_at ASC
`

func (q *Queries) AllChirpsByAuthor(ctx context.Context, userID uuid.NullUUID) ([]Chirp, error) {
	rows, err := q.db.QueryContext(ctx, allChirpsByAuthor, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
